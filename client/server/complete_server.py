#!/usr/bin/env python3
"""
Complete startup script for RetailFlowAI
This script initializes the database, adds products, and starts the Flask server
"""
...
def chatbot():
    ...
    try:
        ...
        # If no products found, provide general message
        if not products:
            response_message = f"I detected a {mood} mood, but I'm still learning about products for this mood. Let me suggest some popular items instead!"
    ...
    if request.method == 'GET':
        ...
        if 'mood' in data:
            ...
            try:
                ...
                cursor.execute('''
                    INSERT INTO products 
                    (name, category, mood_category, price, description, emoji, image_url, brand, rating, tags, is_trending, stock_quantity, ar_enabled)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    ...
                    data.get('stock_quantity', 100), data.get('ar_enabled', True)
                ))
...
def test_server():
    ...
    return jsonify({
        ...
        'message': 'RetailFlowAI backend is ready',
        'endpoints': ['/api/chatbot', '/api/products', '/test']
    })

if __name__ == '__main__':
    print("🚀 Starting RetailFlowAI Server...")
